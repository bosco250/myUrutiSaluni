...
import { MapPin, Building2, Phone, Mail, Globe, Calendar, Users, DollarSign, FileText, X, Navigation, Loader2 } from 'lucide-react';
    ...
    onError: (err: any) => {
      ...
      if (errorData?.message) {
        if (Array.isArray(errorData.message)) {
          ...
          validationErrors.forEach((error: string | { property?: string; constraints?: Record<string, string> }) => {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col slide-in-from-top-4">
          {/* Header */}
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
            <div className="p-6 space-y-8">
              ...
              {Object.keys(errors).filter(key => key !== 'submit' && errors[key]).length > 0 && (
                <div className="p-4 bg-warning/10 border border-warning rounded-xl">
                  <p className="text-warning font-semibold mb-2">Please fix the following errors:</p>
                  <ul className="list-disc list-inside space-y-1">
                    {Object.entries(errors)
                      .filter(([key]) => key !== 'submit' && errors[key])
                      .map(([key, message]) => (
                        <li key={key} className="text-warning text-sm">
                          <span className="font-medium capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}:</span> {message}
                        </li>
                      ))}
                  </ul>
                </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          className={`w-full px-4 py-3.5 bg-background-light dark:bg-background-dark border ${
                            errors.name ? 'border-danger' : 'border-border-light dark:border-border-dark'
                          } rounded-xl text-text-light dark:text-text-dark focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition h-14`}
                          placeholder="Enter your salon's name"
                        />
                        {errors.name && (
                          <p className="text-danger text-sm mt-1.5">{errors.name}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className={`w-full px-4 py-3.5 bg-background-light dark:bg-background-dark border ${
                            errors.businessType ? 'border-danger' : 'border-border-light dark:border-border-dark'
                          } rounded-xl text-text-light dark:text-text-dark focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition h-14 appearance-none cursor-pointer`}
                        >
                        ...
                        {errors.businessType && (
                          <p className="text-danger text-sm mt-1.5">{errors.businessType}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          rows={4}
                          className="w-full px-4 py-3.5 bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark rounded-xl text-text-light dark:text-text-dark focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition resize-none"
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-4 py-3.5 bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark rounded-xl text-text-light dark:text-text-dark focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition h-14"
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="relative">
                          <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-text-light/40 dark:text-text-dark/40" />
                          <input
                            type="tel"
                            required
                            value={formData.phone}
                            onChange={(e) => updateField('phone', e.target.value)}
                            className={`w-full pl-12 pr-4 py-3.5 bg-background-light dark:bg-background-dark border ${
                        ...
                        {errors.phone && (
                          <p className="text-danger text-sm mt-1.5">{errors.phone}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-text-light/40 dark:text-text-dark/40" />
                          <input
                            type="email"
                        ...
                        {errors.email && (
                          <p className="text-danger text-sm mt-1.5">{errors.email}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <Globe className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-text-light/40 dark:text-text-dark/40" />
                          <input
                            type="url"
                            value={formData.website}
                            onChange={(e) => updateField('website', e.target.value)}
                            className="w-full pl-12 pr-4 py-3.5 bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark rounded-xl text-text-light dark:text-text-dark focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition h-14"
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        {formData.latitude && formData.longitude && !locationError && (
                          <div className="mt-2 p-3 bg-success/10 border border-success/20 rounded-lg">
                            <p className="text-success text-sm flex items-center gap-2 mb-1">
                              <MapPin className="w-4 h-4" />
                              <span className="font-medium">Location detected</span>
                            </p>
                            <p className="text-text-light/60 dark:text-text-dark/60 text-xs mb-2">
                              Coordinates: {parseFloat(formData.latitude).toFixed(6)}, {parseFloat(formData.longitude).toFixed(6)}
                            </p>
                            <a
                              href={`https://www.openstreetmap.org/?mlat=${formData.latitude}&mlon=${formData.longitude}&zoom=18`}
                              target="_blank"
                      ...
                      <div>
                        ...
                        <p className="text-xs text-text-light/60 dark:text-text-dark/60 mb-3">
                          Click on the map to set your salon location. The address will be automatically filled.
                        </p>
                        <LocationPicker
                          key={mapKey}
                          ...
                          onReverseGeocode={async (lat, lng) => {
                            try {
                              ...
                              // Don't set error state - the function now returns fallback data
                              // User can still manually edit the address
                            }
                          }}
                          height="400px"
                        />
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-text-light/40 dark:text-text-dark/40" />
                          <input
                            type="text"
                            required
                        ...
                        {errors.address && (
                          <p className="text-danger text-sm mt-1.5">{errors.address}</p>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          />
                          {errors.city && (
                            <p className="text-danger text-sm mt-1.5">{errors.city}</p>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          />
                          {errors.district && (
                            <p className="text-danger text-sm mt-1.5">{errors.district}</p>
                          )}
                        </div>
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="relative">
                            <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-text-light/40 dark:text-text-dark/40" />
                            <input
                              type="date"
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <Users className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-text-light/40 dark:text-text-dark/40" />
                            <input
                              type="number"
                              min="0"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="relative">
                            <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-text-light/40 dark:text-text-dark/40" />
                            <input
                              type="number"
                              min="0"
                              step="1000"
                      ...
                      {/* Social Media */}
                      <div>
                        <label className="block text-sm font-medium text-text-light/80 dark:text-text-dark/80 mb-3">
                          Social Media Links
                        </label>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div>
                            <input
                              ...
                              className="w-full px-4 py-3.5 bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark rounded-xl text-text-light dark:text-text-dark focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition h-14 text-sm"
                      ...
                      {/* Terms Agreement (only for new salons) */}
                      {!salon && (
                        <div className="pt-4">
                          <div className="flex items-start gap-3">
                            <input
                              ...
                              className="mt-1 h-5 w-5 rounded border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark text-primary focus:ring-primary focus:ring-2"
                            />
                            <label
                              htmlFor="terms"
                          ...
                          {errors.agreeToTerms && (
                            <p className="text-danger text-sm mt-1.5 ml-8">{errors.agreeToTerms}</p>
                          )}
                        </div>
                      )}
                    </div>
            ...
            {/* Footer Actions */}
            <div className="sticky bottom-0 p-6 border-t border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark">
              <div className="flex gap-4">
                ...
                {currentStep < totalSteps ? (
                  ...
                  <Button
                    ...
                    className="flex-1"
                  >
                    {mutation.isPending ? 'Saving...' : salon ? 'Update Salon' : 'Register Salon'}
                  </Button>
                )}
              </div>
            </div>
