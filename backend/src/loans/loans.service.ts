...
export class LoansService {
  ...
  async calculateCreditScore(userId: string): Promise<CreditScore> {
    ...
    const appointments =
      await this.appointmentsService.findByCustomerId(userId);
    ...
    const score = Math.max(
      Math.min(baseScore + Math.floor(Math.random() * 100), 1000),
      300,
    );
  ...
  async disburseLoan(
    loanId: string,
    disbursementData: { method?: string; reference?: string },
  ): Promise<Loan> {
    const loan = await this.loansRepository.findOne({ where: { id: loanId } });
    if (!loan) throw new Error('Loan not found');
    ...
    loan.disbursementReference =
      disbursementData.reference || `DISB-${loan.loanNumber}`;
  ...
  async getLoanAuditData(loanId: string, user?: any) {
    ...
    if (applicant.role === UserRole.SALON_OWNER) {
      ...
      if (salonIds.length > 0) {
        ...
        const salesResult = await this.salesService.findBySalonIds(
          salonIds,
          1,
          1000,
        );
        const totalSalesRevenue = salesResult.data.reduce(
        ...
        stats = {
          ...
          averageSalonRevenue:
            salons.length > 0
              ? Number((totalSalesRevenue / salons.length).toFixed(2))
              : 0,
        };
      } else {
        stats = {
      ...
      stats = {
        ...
        averageCommission:
          completed.length > 0
            ? Number((totalEarnings / completed.length).toFixed(2))
            : 0,
      };
    }
    ...
    activityData
      .filter(
        (a) =>
          a.status === 'completed' &&
          new Date(a.scheduledStart) >= sixMonthsAgo,
      )
      .forEach((a) => {
        const month = new Date(a.scheduledStart).toLocaleString('default', {
          month: 'short',
        });
        if (monthlyTrend[month] !== undefined) {
          if (applicant.role === UserRole.SALON_OWNER) {
            ...
            const emp = employeeRecords.find(
              (e: any) => e.salonId === a.salonId,
            );
    ...
    activityData
      .filter(
        (a) =>
          a.status === 'completed' &&
          new Date(a.scheduledStart) >= fourWeeksAgo,
      )
      .forEach((a) => {
    ...
    const peakDay = Object.keys(dayOfWeekStats).reduce((a, b) =>
      dayOfWeekStats[Number(a)] > dayOfWeekStats[Number(b)] ? a : b,
    );
    ...
    const avgMonthlyEarnings =
      applicant.role === UserRole.SALON_OWNER
        ? stats.averageSalonRevenue
        : stats.totalEarnings / 6;

    const dsr =
      avgMonthlyEarnings > 0
        ? (monthlyInstallment / avgMonthlyEarnings) * 100
        : 100;
    ...
    const retentionRate =
      activityData.length > 0
        ? (uniqueCustomers / activityData.length) * 100
        : 0;

    // Stability Factor: Tenure + Completion + Score
    const monthsTenure = Math.floor(
      (now.getTime() - new Date(applicant.createdAt).getTime()) /
        (1000 * 60 * 60 * 24 * 30),
    );
    const completionRate =
      activityData.length > 0
        ? (activityData.filter((a) => a.status === 'completed').length /
            activityData.length) *
          100
        : 0;
    const stabilityScore = Math.min(
      (monthsTenure / 12) * 40 + completionRate * 0.6,
      100,
    );

    const decisionRecommendation =
      dsr < 40 && creditScore.score > 600
        ? 'AUTO_APPROVE'
        : dsr > 70
          ? 'HIGH_RISK_REJECT'
          : 'MANUAL_REVIEW';
    ...
    return {
      ...
      creditScore,
      stats,
      monthlyTrend: trendArray.map((t) => ({
        ...t,
        amount: Number(t.amount.toFixed(2)),
      })),
      highResTrends: {
        daily: Object.entries(dailyStats).map(([date, amount]) => ({
          date,
          amount: Number(amount.toFixed(2)),
        })),
        weekly: Object.entries(weeklyStats).map(([week, amount]) => ({
          week,
          amount: Number(amount.toFixed(2)),
        })),
        peakPerformanceDay: days[Number(peakDay)],
        maxDailyYield: Number(
          Math.max(...Object.values(dailyStats), 0).toFixed(2),
        ),
      },
      decisionSupport: {
        repayment: {
      ...
      performance: {
        totalAppointments: activityData.length,
        completedAppointments: activityData.filter(
