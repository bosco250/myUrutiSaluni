...
export class InventoryController {
  ...
  async updateProduct(
    ...
    if (user.role === UserRole.SALON_OWNER || user.role === UserRole.SALON_EMPLOYEE) {
      const salon = await this.salonsService.findOne(product.salonId);
      if (salon.ownerId !== user.id) {
        throw new ForbiddenException('You can only update products for your own salon');
      }
    }
  ...
  async removeProduct(@Param('id') id: string, @CurrentUser() user: any) {
    ...
    if (user.role === UserRole.SALON_OWNER) {
      const salon = await this.salonsService.findOne(product.salonId);
      if (salon.ownerId !== user.id) {
        throw new ForbiddenException('You can only delete products for your own salon');
      }
    }
  ...
  async findAllMovements(
    ...
    if (user.role === UserRole.SALON_OWNER || user.role === UserRole.SALON_EMPLOYEE) {
      ...
      const salonIds = salons.map(s => s.id);
      if (salonId && !salonIds.includes(salonId)) {
        throw new ForbiddenException('You can only access movements for your own salon');
      }
      // If no salonId specified, filter by all user's salons
      const filteredSalonId = salonId || (salonIds.length === 1 ? salonIds[0] : undefined);
  ...
  async createMovement(@Body() createMovementDto: CreateMovementDto, @CurrentUser() user: any) {
    ...
    if (user.role === UserRole.SALON_OWNER || user.role === UserRole.SALON_EMPLOYEE) {
      ...
      if (salon.ownerId !== user.id) {
        throw new ForbiddenException('You can only create movements for your own salon');
      }
    }
  ...
  async getStockLevels(@Query('salonId') salonId: string | undefined, @CurrentUser() user: any) {
    // Salon owners and employees can only see stock for their salon(s)
    if (user.role === UserRole.SALON_OWNER || user.role === UserRole.SALON_EMPLOYEE) {
      const salons = await this.salonsService.findByOwnerId(user.id);
      const salonIds = salons.map(s => s.id);
      if (salonId && !salonIds.includes(salonId)) {
        throw new ForbiddenException('You can only access stock for your own salon');
      }
