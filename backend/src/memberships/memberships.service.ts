...
import { MembershipApplication, ApplicationStatus } from './entities/membership-application.entity';
import { Membership, MembershipStatus } from './entities/membership.entity';
import { MembershipPayment, PaymentStatus, PaymentMethod } from './entities/membership-payment.entity';
...
export class MembershipsService {
  ...
  async createApplication(applicantId: string, createDto: CreateMembershipApplicationDto): Promise<MembershipApplication> {
    ...
    if (existingApplication) {
      if (existingApplication.status === ApplicationStatus.PENDING) {
        throw new BadRequestException('You already have a pending membership application');
      }
  ...
  async findByApplicantId(applicantId: string): Promise<MembershipApplication | null> {
  ...
  async reviewApplication(
    ...
    if (reviewDto.status === ApplicationStatus.REJECTED && reviewDto.rejectionReason) {
    ...
    if (reviewDto.status === ApplicationStatus.APPROVED) {
      const applicant = await this.usersRepository.findOne({ where: { id: application.applicantId } });
      if (!applicant) {
        throw new NotFoundException(`User with ID ${application.applicantId} not found`);
      }
      
      console.log(`[Membership Approval] Updating user ${applicant.id} (${applicant.email}) role from ${applicant.role} to ${UserRole.SALON_OWNER}`);
      ...
      updatedApplicant = await this.usersRepository.findOne({ where: { id: application.applicantId } });
      
      if (!updatedApplicant) {
        throw new NotFoundException(`User with ID ${application.applicantId} not found after update`);
      }
      
      // Verify the role was updated
      if (updatedApplicant.role !== UserRole.SALON_OWNER) {
        console.error(`[Membership Approval] Failed to update role for user ${application.applicantId}. Expected: ${UserRole.SALON_OWNER}, Got: ${updatedApplicant.role}`);
        throw new Error('Failed to update user role to SALON_OWNER');
      }
      
      console.log(`[Membership Approval] Successfully updated user ${updatedApplicant.id} role to ${updatedApplicant.role} with membership number ${membershipNumber}`);
      
      // Initialize yearly payments for the current year
      try {
        const currentYear = new Date().getFullYear();
        await this.initializeYearlyPayments(application.applicantId, currentYear);
        console.log(`[Membership Approval] Initialized yearly payments for ${currentYear}`);
      } catch (error) {
        console.error('[Membership Approval] Failed to initialize payments:', error);
  ...
  async updateMembership(id: string, updateDto: UpdateMembershipDto): Promise<Membership> {
  ...
  async createPayment(createDto: CreateMembershipPaymentDto): Promise<MembershipPayment> {
    const member = await this.usersRepository.findOne({ where: { id: createDto.memberId } });
    if (!member) {
    ...
    if (createDto.dueDate) {
      ...
      dueDate = new Date(createDto.paymentYear, createDto.installmentNumber === 1 ? 0 : 6, 1);
    }
  ...
  async recordPayment(recordDto: RecordPaymentDto, recordedById: string): Promise<MembershipPayment> {
    ...
    if (!payment) {
      throw new NotFoundException(`Payment with ID ${recordDto.paymentId} not found`);
    }
  ...
  async findPaymentsByMember(memberId: string, year?: number): Promise<MembershipPayment[]> {
  ...
  async getPaymentStatus(memberId: string, year: number): Promise<{
  ...
  async initializeYearlyPayments(memberId: string, year: number): Promise<MembershipPayment[]> {
