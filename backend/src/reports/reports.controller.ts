import { Controller, Get, Param, Query, UseGuards, Res, ForbiddenException } from '@nestjs/common';
...
export class ReportsController {
  ...
  async generateSalesReport(
    ...
    if (user.role === UserRole.SALON_OWNER || user.role === UserRole.SALON_EMPLOYEE) {
      ...
      if (salonId && !salonIds.includes(salonId)) {
        throw new ForbiddenException('You can only generate reports for your own salon');
      }
  ...
  async generateMembershipCertificate(
    ...
    if (user.role !== UserRole.SUPER_ADMIN && user.role !== UserRole.ASSOCIATION_ADMIN) {
      if (userId !== user.id) {
        throw new ForbiddenException('You can only generate your own membership certificate');
      }
    }

    try {
      const pdf = await this.reportsService.generateMembershipCertificate(userId);
      ...
      console.error('Error in generateMembershipCertificate controller:', error);
