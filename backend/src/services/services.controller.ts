...
export class ServicesController {
  ...
  async create(@Body() createServiceDto: CreateServiceDto, @CurrentUser() user: any) {
    ...
    if (
      ...
      if (salon.ownerId !== user.id) {
        throw new ForbiddenException('You can only create services for your own salon');
      }
    }
  ...
  async findAll(@Query('salonId') salonId: string | undefined, @CurrentUser() user: any) {
    ...
    if (user.role === UserRole.SALON_OWNER || user.role === UserRole.SALON_EMPLOYEE) {
      ...
      if (salonId && !salonIds.includes(salonId)) {
        throw new ForbiddenException('You can only access services for your own salon');
      }
  ...
  async update(
    ...
    if (user.role === UserRole.SALON_OWNER || user.role === UserRole.SALON_EMPLOYEE) {
      const salon = await this.salonsService.findOne(service.salonId);
      if (salon.ownerId !== user.id) {
        throw new ForbiddenException('You can only update services for your own salon');
      }
    }
  ...
  async remove(@Param('id') id: string, @CurrentUser() user: any) {
    ...
    if (user.role === UserRole.SALON_OWNER) {
      const salon = await this.salonsService.findOne(service.salonId);
      if (salon.ownerId !== user.id) {
        throw new ForbiddenException('You can only delete services for your own salon');
      }
    }
