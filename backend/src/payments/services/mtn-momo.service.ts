...
export class MtnMomoService {
  ...
  async requestPayment(params: {
    ...
    this.logger.log(
      `[MOCK] Requesting payment from ${params.phoneNumber} for ${params.amount} RWF`,
    );
    ...
    setTimeout(() => {
      ...
      if (payment) {
        ...
        this.logger.log(
          `[MOCK] Payment ${referenceId} completed with status: ${payment.status}`,
        );
      }
  ...
  async requestRefund(params: {
    ...
    this.logger.log(
      `[MOCK] Refund requested for ${params.originalReferenceId}, amount: ${params.amount}`,
    );
