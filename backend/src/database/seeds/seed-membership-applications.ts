...
export async function seedMembershipApplications(dataSource: DataSource) {
  ...
  console.log(
    `📊 Current database state: ${existingAppsCount} existing application(s)`,
  );
  ...
  if (salonOwners.length === 0) {
    console.log(
      '⚠️  No salon owners found. Run salon seeds first: npm run seed:salons',
    );
    return;
  }
  ...
  const applicationData = [
    {
      ...
      businessDescription:
        'Modern hair studio specializing in contemporary styles and treatments',
    ...
    },
    {
      ...
      businessDescription:
        'Quick and affordable beauty services for busy professionals',
    ...
    },
    {
      ...
      businessDescription:
        'Full-service spa and beauty salon with relaxation treatments',
    ...
    },
    {
      ...
      businessDescription:
        'Professional barbershop serving the Southern Province',
    ...
    },
    {
      ...
      businessDescription:
        'Specialized in natural hair care and organic products',
    ...
    },
    {
      ...
      businessDescription:
        'Trendy salon offering the latest international styles',
  ...
  for (const appData of applicationData) {
    try {
      ...
      if (existing) {
        skipped++;
        console.log(
          `  → Skipping existing application: ${appData.businessName}`,
        );
        continue;
      }
      ...
      const randomOwner =
        salonOwners[Math.floor(Math.random() * salonOwners.length)];
      ...
      created++;
      console.log(
        `  ✓ Created application: ${application.businessName} (${application.status})`,
      );
    } catch (error) {
      console.error(
        `  ✗ Error creating application for ${appData.businessName}:`,
        error.message,
      );
    }
  }
